...
** *연동된 구독*: 기본 구독에 연결된 보조 구독이 있는 경우, 이 열에는 기본 구독에 연결된 구독 번호가 표시됩니다. 이 열은 기본적으로 숨겨져 있으며, 연결된 구독이 있는 경우에만 표시됩니다.
** *Keystone 버전*: 구독에 사용되는 Keystone 서비스의 버전입니다. 성능 서비스 수준에 대한 요금제 규칙은 구독 버전에 따라 다를 수 있습니다. 버전 v1에 대한 자세한 내용은 https://docs.netapp.com/us-en/keystone/index.html["Keystone 구독 서비스 | 버전 1 문서"^]를 참조하십시오. 이 열은 기본적으로 숨겨져 있으며 열 선택기 image:column-selector.png["열 선택기 아이콘"] 아이콘을 사용하여 활성화할 수 있습니다.
