...
+
*Hide/Show Columns*를 사용하여 표에 표시되는 열을 추가하거나 제거할 수 있습니다. 기본적으로 표에는 이전에 저장한 열 기본 설정이 표시됩니다. *Department* 또는 *Total Footprint* 열과 같이 새로 추가된 열은 기본적으로 숨겨져 있으며 표에 표시하려면 수동으로 선택해야 합니다. 모든 열을 선택하거나 선택 해제할 수 있으며, 기본 설정은 나중에 사용할 수 있도록 저장됩니다. CSV 보고서를 다운로드할 때는 표시 기본 설정과 관계없이 사용 가능한 모든 열이 내보내기에 포함됩니다.
