...
NOTE: Keystone 구독의 용량 사용량은 대시보드와 보고서에서 TiBs 단위로 표시되며 소수점 둘째 자리까지 반올림됩니다. 사용량이 0.01 TiB 미만인 경우 값은 0 또는 사용량 없음으로 표시됩니다. 이러한 화면의 데이터는 UTC 시간(서버 시간대)으로 표시됩니다. 쿼리할 날짜를 입력하면 자동으로 UTC 시간으로 간주됩니다. 사용량 측정항목에 대한 자세한 내용은 link:../concepts/metrics.html#metrics-measurement["측정 지표"]을 참조하십시오. Keystone에서 사용되는 다양한 용량에 대한 자세한 내용은 link:../concepts/supported-storage-capacity.html["Keystone의 스토리지 용량 유형"]를 참조하십시오.
