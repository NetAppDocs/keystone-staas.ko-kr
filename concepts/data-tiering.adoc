...
[role="lead"]
Keystone STaaS의 파일 및 블록 스토리지 표준 서비스에는 사용 빈도가 낮은 데이터를 식별하고 Keystone STaaS에서 지원하는 NetApp 콜드 스토리지로 계층화하는 계층화 기능이 포함되어 있습니다. 콜드 데이터를 Keystone STaaS에서 지원하는 비NetApp 스토리지로 계층화하려면 데이터 계층화 기능을 추가 서비스로 사용할 수 있습니다. 이 추가 서비스는 통합 데이터 스토리지 유형에서만 지원됩니다.
...
NOTE: 계층화 추가 서비스는 Amazon Web Services(AWS) S3, Azure Blob, Google Cloud Platform(GCP) 및 Keystone STaaS에서 지원하는 S3 호환 타사 객체 스토리지와 같은 NetApp 이외의 스토리지로 데이터를 계층화할 때만 필요합니다. 이 서비스는 통합 데이터 스토리지 유형에서만 사용할 수 있습니다.
